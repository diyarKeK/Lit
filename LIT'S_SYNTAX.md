...
Чтобы сделать первую программу на Lit:
    a. Создайте файл с расширеннием .lit
    b. Напишите туда:
...
Функция 'main()' - это точка входа в вашу программу. Слово 'fun' обозначает что мы объявляем функцию.
...
Как мы видим, чтобы обозначить строку нужно использовать одинарные кавычки ''. Также мы видим что нам вообще не нужно писать ';' в конце строчки

### 3. Переменные
В Lit существует только 4 стандартных типа переменных:
  a. int - целочисленное число
  b. float - число с плавующей точкой
  c. bool - булевое значение
  d. str - строка
Чтобы объявить переменную, укажите тип переменной, дальше ее имя, потом знак присвоения '=', а потом уже ее значение:
...
### 4. Комментарии
Чтобы поставить комментарий в коде, укажите '//' перед комментарием:
...
### 5. Выделение памяти
Как мы знаем лучше не использовать обычный int с 32 битным выделением памяти для чисел которые будут итак маленькими.
В Lit есть много выделений памяти для простых типов int и float:
    для int используются с типом i и u, их отличие в том что i может содержать отрицательные числа, а u нет:
    ...
    Обычный int 32 Битное число, которая может содержать отрицательные числа, то есть - i32.
    Поэтому лучше писать не писать:
    ...
    a. f32 - 32 Битное число с плавующей точкой
    b. f64 - 64 Битное число с плавующей точкой

    Обычный float использует f32

#### Примечание:
    выделение памяти только для int и float
...
f32 - до 7 знаков после запятой
f64 - до 15 знаков после запятой
...
### 6. if-else конструкции
Чтобы сделать if-else конструкцию, укажите ключевое слово if, затем условие, затем откройте фигурные скобки и напишите внутри нее тело конструкции, которые выполняться если if истина. Запишите else if для следующего условия или else если все условия были false

В условиях if-else конструкций (да и в условий while циклов конечно же) можно использовать:
...
### 7. Очистка памяти
У Lit есть свой авто сборщик мусора (GC), чтобы программисты не морочили голову над очисткой памяти
...
fun main() {
  ...
  int sum1 = sum(1, 9, 3, 2) // 15
  int sum2 = sum(3, 2, 82, -1) // 86
}
...
// Чтобы функция принимала любое количество аргументов, поставьте перед типом переменной три точки - '...'
fun sum(int... nums): int {
  // int... это как int[] 
  int sum = 0
  for num in nums {
    sum += num
...
fun main() {
  str s = input('Введите текст: ') // Можно написать что выведется на экран
  print('Вы ввели: ' + s)
}

### 11. Циклы while и for
Чтобы сделать цикл, введите какой это
цикл (while, for), дальше введите 
условия при которых цикл будет 
выполнятся:
...
### 12. Массивы и работа с ними
Чтобы создать массив, указажите 
сначало тип переменной, дальше '[]'
до названия переменной:
...
fun main() {
  ...
  // как видим, нам не нужно писать каким типом будет переменная 'el'
...
### 14. Импорты
Чтобы импортировать класс, укажите 
ключевое слово import в файле, дальше
её package:
...
Также можно импортировать класс как:
...
fun main() {
  ...
  if not Str.is_digit(str_num) {
    throw new RE('Not Digit!')
    // и выходим автоматически из программы, так как написан throw
  } else {
    ...
  }
}

а try-catch конструкции блокируют 
throw, только выводят в консоль 
текст с ошибкой.
...
### 17. Разные строки кода в одной строке кода
...
} // this - означает что мы обращаемся к полю экземпляра
...
заменятся на дефолтное значение:
...
Чтобы объявить interface, укажите
...
fun main() {
  ...
  // Error At *.lit:18: Something is Wrong
}

### 22. instance_of
ключевое слово instance_of возвращает
булевое значение 'bool', и определяет
...
class MyList<E> { // принимаем дженерик, пометив его как E (сокращение от слово Element) для наглядности 
  ...
  fun sort(lambda: (E[] array) method) {
    method(current_array)
  }
...
fun main() {
  int res = accept_lambda((a, b) => {
    return a * a + 2 * a * b + b * b
  }, 2, 3) // Не нужно писать в лямбде выражении какими типами будут a и b, так как типы a и b уже определены в лямбде выражения some
  // Также можно как угодно называть переменные a и b, так как их типы определены и тд
...
// Возможно тут могут быть не все 
// ключевые слова
  a. int
  b. float
  c. bool
